function analysesnaphist_auto_prime_2(fname,numcolors, fname_title,handles)
...
for i=1:length(colorsuffixes)
    if length(dir([handles.current_directory, '\',handles.outputfolder,'\',colorsuffixes{i}])) >0;
...
%for d = 1:length(prefixes)
        plothist2(prefixes{1},presub{1},colornames,usedcolors,fname_title,handles)
%end
...
function plothist2(input,functnm,colornames,usedcolors,fname_title,handles)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
var_len=length(who);
load([handles.current_directory, '\',handles.outputfolder,'\',input]);
...
dis = sqrt((cen(:,1)-1020).^2+(cen(:,2)-1020).^2); %d = Distance from center of image
cirIndex = dis<800; %Index those are within 400 pixels of center (gives out a logical matrix)
...
figure;
%set(gcf,'Position',[680 440 672 715]);
set(gcf,'Position',[680 440 750 715]);
for i=1:numcolors
    fpd = meansflatcorr(:,i);
    if min(fpd)<0
...
print(gcf,'-djpeg',[handles.current_directory, '\',handles.outputfolder,'\',input2,'.jpg']);
saveas(gcf,[handles.current_directory, '\',handles.outputfolder,'\',input2],'fig');
